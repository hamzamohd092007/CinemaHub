...
userRouter.post('/signup', upload.single("avatar"), async (req, res) => {
    try {
        const { fullName, email, language, password } = req.body;
        if (!fullName || !email || !language || !password) {
        ...
        const user = await User.create({ fullName, ...(avatar && { avatar }), email, language, password: hashed });
...
userRouter.put('/watchlist', auth, async (req, res) => {
    try {
        const userId = req.user.userId;
        if (!userId) {
            return res.status(401).json({ success: false, message: "Unauthorized" });
...
userRouter.put('/change-language', auth, async (req, res) => {
    try {
        ...
        if (!language) {
            return res.status(400).json({ success: false, message: "Language not chosen" });
        }
        const user = await User.findById(userId);
        if (!user) {
...
userRouter.put('/show-adult', auth, async (req, res) => {
    try {
        const userId = req.user.userId;
        if (!userId) {
            return res.status(401).json({ success: false, message: "Unauthorized" });
